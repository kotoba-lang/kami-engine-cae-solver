(ns run-calculix-contact-mesh-study
...
(defn- file-evidence [root path]
  (let [file (io/file root path) digest (MessageDigest/getInstance "SHA-256")]
    (when-not (.isFile file) (throw (ex-info "contact mesh output missing" {:path path})))
...
(defn- run-level [root image {:keys [id divisions h-relative]}]
  (let [name (name id) dir (io/file root name) _ (.mkdirs dir)
        generated (mesh/contact-blocks-input {:divisions divisions})
        ...
        pressure (evidence/calculix-contact-pressure (slurp (io/file dir (str name ".dat"))))
        paths [(str name ".inp") "log.ccx" (str name ".dat") (str name ".frd")
               (str name ".sta") (str name ".cvg")]
        passed? (and (zero? (:exit run)) (:complete? log) (= 1.0 (:time pressure))
                     (pos? (:sample-count pressure)))]
    (merge {:id id :divisions divisions :h-relative h-relative :nodes (:nodes generated)
            ...
            :passed? passed? :value (:maximum-pressure pressure)} pressure)))
...
(defn -main [& _]
  (let [manifest (-> "cae/external-solvers.edn" io/resource slurp edn/read-string :calculix-2.21-arm64)
        ...
        root (.getCanonicalFile (io/file (or (System/getenv "CAE_EXTERNAL_RUN_DIR")
                                             ".cache/external-solvers/calculix-contact-mesh")))
        ...
        study (evidence/contact-pressure-sensitivity {:levels run-levels :local-target 0.05})
        envelope {:case-id "calculix-2.21-contact-pressure-mesh-sensitivity"
                  ...
                  :status (if (:evidence-passed? study) :external-contact-sensitivity-verified
                              :external-contact-sensitivity-rejected)}]
    (when-not (:passed? envelope) (throw (ex-info "contact mesh evidence rejected" envelope)))
