...
(defn -main [& _]
  (let [manifest (-> "cae/external-solvers.edn" io/resource slurp edn/read-string :openfoam-v2506-arm64)
        ...
        root (.getCanonicalFile (io/file (or (System/getenv "CAE_EXTERNAL_RUN_DIR")
                                             ".cache/external-solvers/openfoam-sbse-rans")))
        ...
        case (rans/case-files rans/default-experiment)
        _ (when-not reuse?
        ...
        yplus (some->> (re-find #"patch floorBump y\+ : min = ([0-9.Ee+-]+), max = ([0-9.Ee+-]+), average = ([0-9.Ee+-]+)" log-text)
                       rest (mapv number))
        shear (some->> (re-find #"min/max\(floorBump\) = \(([-0-9.Ee+ ]+)\), \(([-0-9.Ee+ ]+)\)" log-text)
        ...
        checks {:mesh-ok? mesh-ok? :nonorthogonality-below-40-deg? (< max-nonorth 40.0)
        ...
        envelope {:case-id "openfoam-v2506-nasa-sbse-3d-coarse-komega-sst"
